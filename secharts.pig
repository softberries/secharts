REGISTER sopiggy.jar;

DEFINE FIRST_ANSWER_AWAIT_TIME com.softwarepassion.sopiggy.FIRST_ANSWER_AWAIT_TIME();
DEFINE ROW_COUNT(X) RETURNS Z { Y = group $X all; $Z = foreach Y generate COUNT($X); };

--"posts input: file:/Users/kris/Downloads/StackExchangeDataDumpAug2012/Content/mathematica.stackexchange.com/posts{1,2}.xml'
--"users input: file:/Users/kris/Downloads/StackExchangeDataDumpAug2012/Content/mathematica.stackexchange.com/users.xml'
--"Usage: pig -f script.pig -param posts=somefile.txt"

DBPosts = LOAD '$posts' using com.softwarepassion.sopiggy.SoCustomPostsLoader() AS (
  id:chararray,
  parentId:chararray,
...
DBUsers = LOAD '$users' using com.softwarepassion.sopiggy.SoCustomUsersLoader() AS (
  id:chararray,
  reputation:long,
  creationDate:chararray,
